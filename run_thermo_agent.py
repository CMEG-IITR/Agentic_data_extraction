...
# === Node 8: Judge verification node ===
...
def judge_node(state: State) -> State:
    ...
    try:
        print(f"🧠 Running property-level LLM Judge for {folder.name}...")
        ...
        with open(log_path, "a", encoding="utf-8") as log:
            ...
            log.write("=" * 60 + "\n")
        print(f"⚠️ Judge node crashed in {folder.name}: {e}")
...
graph.add_node("Judge_verification", judge_node)
...
for folder in base_dir.iterdir():
    ...
    # --- Short delay between each paper ---
    t = random.uniform(6, 10)
    print(f"⏳ Sleeping for {t:.2f} seconds after {folder.name} before next folder...")
    time.sleep(t)

    # --- Long cooldown every 10 papers ---
    if new_count % 10 == 0:
        print(f"🕐 Processed {new_count} papers — cooling down for 60 seconds...")
        time.sleep(60)
